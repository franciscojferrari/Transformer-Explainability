...
class BertForSequenceClassificationExplanator:
    ...
    def generate_explanation(
        ...
        **input
        ...
        logits = output.logits.reshape(
            input["input_ids"].shape[0], self.bert_model.config.num_labels
        )
        if index is None:
            one_hot = torch.nn.functional.one_hot(
                torch.argmax(logits, dim=1), self.bert_model.config.num_labels
            ).detach()
        else:
        ...
        for attention_block in self.bert_model.bert.encoder.layer:
            ...
            weighted_layer_attention_relevance = torch.eye(rel.shape[-1]).to(
                rel.device
            ) + (rel_grad * rel).clamp(min=0).mean(dim=1)
            if weighted_attention_relevance is None:
                weighted_attention_relevance = (
                    weighted_layer_attention_relevance.double()
                )
            else:
                weighted_attention_relevance = torch.bmm(
                    weighted_attention_relevance,
                    weighted_layer_attention_relevance.double(),
                )
    ...
    def vizualize(
        self, explanations, tokens, pred, label, label_string, label_being_explained=1
    ):
        for i in range(explanations.shape[0]):
            print(
                [
                    (tokens[i][j], explanations[i, j].item())
...
if __name__ == "__main__":
    ...
    model = BertForSequenceClassification.from_pretrained(
        huggingface_model_name, num_labels=2
    )
    print("Using activation func: ", model.config.hidden_act)
    explanator = BertForSequenceClassificationExplanator(model)
    exp, pred = explanator.generate_explanation(
        **inputs, normalize_scores=True, get_one_hot=True
    )
    explanator.vizualize(
        exp, tokens, torch.argmax(pred, dim=1), torch.tensor([1, 0]), ["NEG", "POS"]
    )
    # Notebook example
