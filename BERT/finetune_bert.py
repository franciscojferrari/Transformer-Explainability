...
from transformers import AutoTokenizer, get_scheduler, AdamW
...
class BertSequenceClassificationSystem(pl.LightningModule):
    def __init__(self, huggingface_model_name, config=None):
        ...
        if config is None:
            ...
            self.bert_classifier = BertForSequenceClassification.from_pretrained(
                huggingface_model_name, config=config
            )
    ...
    def training_step(self, batch, batch_idx):
        ...
        if y.logits.dim() - 1 != batch["labels"].dim():
            ...
            self.log(
                "train_accuracy",
                self.acc(torch.argmax(F.softmax(y.logits, dim=1), dim=1, keepdim=True)),
                batch["labels"],
            )
        return y.loss
    ...
    def validation_step(self, val_batch, val_batch_idx):
        ...
        if y.logits.dim() - 1 != val_batch["labels"].dim():
            logits = logits.squeeze(1)
        self.log("val_f1", self.f1(logits, val_batch["labels"]))
        self.log(
            "val_accuracy",
            self.acc(
                torch.argmax(F.softmax(logits, dim=1), dim=1), val_batch["labels"]
            ),
        )

    def test_step(self, test_batch, test_batch_idx):
        torch.set_grad_enabled(True)
        explanation, one_hot_pred, logits = self.explanator.generate_explanation(
            **test_batch, get_logits=True
        )
        np.savetxt(
            "BERT/BERT_explanations/{}.csv".format(test_batch_idx),
            explanation.detach().cpu().numpy(),
        )
        class_pred = torch.argmax(one_hot_pred, dim=1)
        np.savetxt(
            "BERT/BERT_preds/{}.csv".format(test_batch_idx),
            class_pred.detach().cpu().numpy(),
        )
        np.savetxt(
            "BERT/BERT_logits/{}.csv".format(test_batch_idx),
            logits.detach().cpu().numpy(),
        )

    def configure_optimizers(self):
        optimizer = AdamW(
            self.parameters(), lr=self.bert_classifier.config.evidence_classifier["lr"]
        )
        scheduler = get_scheduler(
            ...
            optimizer=optimizer,
            num_warmup_steps=self.bert_classifier.config.evidence_classifier[
                "warmup_steps"
            ],
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Custom BERT model for pytorch using pytorch lightning. Note that this code does NOT produce the same results as Chefer et al."
    )
    parser.add_argument(
        "-p", "--bert-params", type=str, help="Path to bert params (json file)."
    )
    parser.add_argument(
        "-c",
        "--pytorch-lightning-checkpoint-dir",
        type=str,
        help="Path to the pytorch checkpoint directory (Can be a brand new or earliear directory).",
    )
    parser.add_argument(
        "-ckpt",
        "--pytorch-lightning-checkpoint-path",
        type=str,
    ...
    parser.add_argument(
        "-g", "--gpus", type=int, help="Choose amount of gpus to execute the model with"
    )
    parser.add_argument(
        "-b",
        "--bert-dir",
        type=str,
        help="Set the bert dir to load the huggingface model from. Can not specify -ckpt and -b at the same time.",
    )
    parser.add_argument(
        "-n",
        "--num-dataloader-workers",
        type=int,
        help="Set the num_workers of the DataLoader.",
    )
    parser.add_argument(
        "-is", "--inference-split", type=str, help="The data split to use for inference"
    )
    ...
    if args.train:
        ...
        train_dataset, val_dataset = load_dataset(
            "movie_rationales", split=["train", "validation"]
        )
        ...
        def label_parse(examples):
            return {
                "label": [
                    1 if examples["label"][i] == 1 else 0
        ...
        train_dataloader = DataLoader(
            ...
            num_workers=1
            if args.num_dataloader_workers is None
            else args.num_dataloader_workers,
        )
        ...
        checkpoint_callback = pl.callbacks.ModelCheckpoint(
            monitor="val_loss",
            dirpath=args.pytorch_lightning_checkpoint_dir,
            filename="BERT-ERASER-Movies-{epoch:02d}-{val_loss:.2f}",
            save_top_k=1,
            mode="min",
        )
        bert_system = BertSequenceClassificationSystem(
            model_config.bert_dir if args.bert_dir is None else args.bert_dir,
            config=model_config,
        )
        trainer = pl.Trainer(
            ...
            gradient_clip_val=model_config.evidence_classifier[
                "max_grad_norm"
            ],  # Defaults to gradient_clip_algorithm="norm"
        ...
        try:
            trainer.save_checkpoint(
                os.path.join(args.pytorch_lightning_checkpoint_dir, "final_checkpoint")
            )
        except:
            pass
        ...
        try:
            ...
            pass
        try:
            os.mkdir("BERT/BERT_results/BERT_explanations")
        except FileExistsError:
            pass
        try:
            os.mkdir("BERT/BERT_results/BERT_preds")
        except FileExistsError:
            pass
        try:
            os.mkdir("BERT/BERT_results/BERT_logits")
        except FileExistsError:
            pass
        ...
        if args.bert_dir is not None:
            bert_system = BertSequenceClassificationSystem(
                model_config.bert_dir, config=model_config
            )
            ...
            if args.gpus is None:
                state_dict = torch.load(
                    args.pytorch_lightning_checkpoint_path,
