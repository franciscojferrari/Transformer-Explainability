    ...
    * decode first the evidence, then run classification for each split
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if epoch - best_epoch > patience:
            ...
            torch.save(epoch_data, epoch_save_file)
            break
    print("Doing some weird loading")
    epoch_data['done'] = 1
    ...
    if os.path.exists(epoch_save_file):
        ...
        # explainability
        method = "transformer_attribution_reproduced"
        explanations = BertForSequenceClassificationExplanator(test_classifier)
        method_folder = {"transformer_attribution_reproduced": "ours"}
        method_expl = {"transformer_attribution_reproduced": explanations.generate_explanation}
        ...
        for batch_start in range(0, len(test), test_batch_size):
            print("Doing batch {}".format(batch_start + 1))
...
\end{document}
                ...
                if method in ["transformer_attribution", "partial_lrp", "attn_gradcam", "lrp"]:
                    ...
                    cam_false_class = cam_false_class[0]
