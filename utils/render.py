...
def repaint_corner_pixels(rgbimg, scaling=3):
    ...
    rgbimg[0:scaling, 0:scaling, :] = (
        rgbimg[0, scaling, :] + rgbimg[scaling, 0, :] + rgbimg[scaling, scaling, :]) / 3.0
    # bottom right corner
    rgbimg[-scaling:, -scaling:, :] = (rgbimg[-1, -1 - scaling, :] + rgbimg[-1 - scaling, -1,
                                       :] + rgbimg[-1 - scaling, -1 - scaling, :]) / 3.0
    return rgbimg
...
def digit_to_rgb(X, scaling=3, shape=(), cmap='binary'):
    ...
    image = cmap(image.flatten())[..., 0:3].reshape(
        [image.shape[0], image.shape[1], 3])  # colorize, reshape
        ...
        if not sz[0] == rgb_images[i].shape[0] and sz[1] == rgb_images[i].shape[2]:
            ...
            print(
                'got     : Hx_xD = {0}x_x{1}'.format(
                    rgb_images[i].shape[0],
                    rgb_images[i].shape[1]))
